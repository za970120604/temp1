...
void init_threads() {
    ...
    for (int i = 0; i < MAX_THREAD_COUNT; i++) {
        ...
        thread_descriptors[i].prog = NULL;
...
void thread_exec(void* prog, unsigned int prog_size){
    disable_interrupt();
    ...
    if (prog_size > 0){
        ...
        utils_memcpy(prog, cur_thread->prog, prog_size);
    }
    else {
    ...
    exec_in_el0(cur_thread->prog, cur_thread->user_sp + USER_STK_SIZE); 
}
