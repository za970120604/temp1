...
static unsigned int total_allocated_frame = 0;
...
void buddy_free(int block_index) { // input: the memory block index to be freed & the order of this memory block, assume the input information is correct
    ...
    while (order < MAX_ORDER && 
		...
		uart_send_string("\r\n[ERROR] Corrupted free list structure detected in buddy_free\r\n");
	    }
        }
...
void print_memory_stats() {
    ...
    uart_send_string("Allocated: ");
    uart_binary_to_hex(total_allocated_frame);
    uart_send_string(" frame\r\n");
    
    uart_send_string("Free blocks by order:\r\n");
    for (int i = 0; i <= MAX_ORDER; i++) {
        int count = 0;
        buddy_t* block = free_frame_lists[i];
        while (block != BUDDY_NULL) {
            count++;
            block = block->next;
        }
        ...
        if (count > 0) {
            uart_send_string("Order ");
            uart_binary_to_hex(i);
            uart_send_string(": ");
            uart_binary_to_hex(count);
            uart_send_string("\r\n");
        }
    }
